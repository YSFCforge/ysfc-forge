...
## Förord — hur dokumentet ska läsas

Detta är recovery/master-kontexten för integrationsprojektet **SysEx Forge**.

SysEx Forge är inte själva Y2L-formatmappningsprojektet. Target-formatet Y2L dokumenteras i den separata **YSFC Forge**-dokumentationen. SysEx Forge ligger ovanpå detta formatarbete och svarar på en annan fråga:

> Hur tar vi en Yamaha Soundmondo SysEx Performance från flera Yamaha-generationer och skapar en strukturellt giltig modern Y2L Performance med hög fidelitet?

Dokumentet fokuserar därför på:

1. SysEx-källfamiljer och blocklayouter;
2. source→normaliserad mapping;
3. source→target-skillnader mellan generationer;
4. waveform/dependency-resolution;
5. Y2L-writer-integration;
6. ESP-verifierade konverterings-checkpoints;
7. fail-closed-regler och olösta områden.

### Evidensprioritet

1. **ESP_VERIFIED** — generated Y2L loaded in MODX M ESP and the relevant UI/sound state was inspected.
2. **★★★★★ Binärverifierat** — controlled single-edit diff or equivalent direct binary evidence.
3. **★★★★☆ Yamaha-härlett** — official Yamaha Data List / MIDI bulk table plus structural confirmation.
4. **★★★☆☆ Härlett/antaget** — strong pattern but not directly verified.
5. **[STRUCT]** — structure known, semantics incomplete.
6. **[UNKNOWN]** — ska inte skrivas eller hittas på.

Uppgradera aldrig ett härlett fält till verifierat enbart för att filen laddar.

---

# 1. Projektets syfte

SysEx Forge konverterar Yamaha Soundmondo SysEx Performances into modern Y2L Performances usable by:
...
Projektet är ett interoperabilitetslager, inte en SysEx-librarian och inte en generell MIDI-editor.

---

# 2. Relation till YSFC Forge

Projekten delar kod och forskning men har separata ansvarsområden.
...
# 4. Identifiering av source family

Canonical routing är:

```text
modelId → source family → engine/Part topology → mapping path → Y2L writer
```

Använd inte en separat `sourceProfile` control path when Yamaha model identity is available.
...
# 17. SysEx Converter versionshistorik
...
# 20. Companion library-hantering
...
# 24. Canonical aktuella regler

För snabb recovery är detta de icke-förhandlingsbara aktuella fakta:
...
När en regel ändras efter ny ESP-evidens ska **REFERENCE och FULL_CONTEXT uppdateras tillsammans**, och den äldre regeln ska uttryckligen markeras som ersatt.

---

# 26. Parameter-level master byte-map recovery

`SYSEX_FORGE_REFERENCE_sv.md` är nu den auktoritativa **parameter-för-parameter byte-kartan** för SysEx Forge. Den innehåller hela 153-punkters FM-X source→target-matrisen, normaliserade cross-generation source-fält, explicita AWM2/AN-X/Arp/FX source-offsets, aktuella AWM2/FM-X/AN-X/Drum Y2L target-maps samt Control Assign, Sidechain, Portamento och EPFM-integritetsregler.

## 26.1 Source- och target-evidens hålls separerade

```text
Soundmondo/WebMIDI block + source offset
                ↓
        normaliserat semantiskt fält
                ↓
Y2L blob/Part/engine/record target offset
```

En rad är fullt cross-mappad först när båda sidor har evidens. Saknade source- eller target-koordinater lämnas tomma i stället för att infereras.

## 26.2 Recovery-källhierarki

Master byte map byggdes om från:

1. `FMX_COVERAGE_MATRIX_v177.csv` — 153 FM-X source→destination parameterpunkter.
2. `normalized_parameter_map.json` — legacy/M-generation Performance- och Part-sourcefält.
3. aktuell `sysex_parser.py` — explicita AWM2/AN-X/Arp/FX source-koordinater och codecs.
4. Control Assign/Sidechain recovery-specifikationen från 2026-08-12.
5. aktuella Performance Editor/serializer-konstanter — moderna Y2L AWM2/FM-X/AN-X/Drum-targets.
6. ESP-verifierade Portamento- och EPFM-boundary-experiment.

## 26.3 Underhållsregel

För varje framtida mapping ska kod/mappingdata, `SYSEX_FORGE_REFERENCE_sv.md` (exakta koordinater/encoding) och `SYSEX_FORGE_FULL_CONTEXT_sv.md` (experiment/resonemang/status) uppdateras tillsammans. Reference är första stället att titta när frågan är **”vilken byte är den här parametern?”**.
