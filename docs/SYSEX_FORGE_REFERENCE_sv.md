# SysEx Forge — Teknisk referens / Master Byte Map
...
## 1. Cross-generation normaliserad Performance/Part-map
...
## 2. Ytterligare explicita Soundmondo/WebMIDI source-offsets

Detta är source-koordinater som explicit avkodas av aktuell parser/recovery. De är inte konstruerade från target-offsets.
...
Övriga råa sidechain-koder måste bevaras tills de klassificerats; anta inte att varje kod är ett Part-nummer.
...
## 6. FM-X verifierad source → target-täckning (komplett v1.0.77-matris)

Detta avsnitt innehåller avsiktligt hela recovery coverage-matrisen i stället för en sammanfattning. Den innehåller **153 spårade parameterpunkter**.
...
## 7. FM-X target-hjälpkartor från aktuell editor
...
**Korrigering:** Common `+41` är Assignable Switch, **inte Portamento**.
...
## 12. Modern EPFM container-fakta
...
## 13. Vad denna Reference *inte* påstår

- En target-koordinat från aktuell Editor/serializer bevisar **inte** i sig motsvarande Soundmondo source-koordinat.
- En Yamaha-dokumenterad source-koordinat bevisar **inte** i sig Y2L-encodingen.
- Filabsoluta offsets från kontrollerade tester är evidenskoordinater, inte produktionskonstanter.
- Fields marked only as current implementation/mapped target should be kept distinct from `ESP_VERIFIED` rows.

När ett nytt kontrollerat test identifierar en source- eller target-byte ska den läggas till här som en ny rad i stället för att fylla luckan genom analogi.
